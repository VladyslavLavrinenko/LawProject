...
let allTexts = document.getElementsByClassName('services__text')

let buttonsArray = [];
let textsArray = []
for(let i = 0;i < allButtons.length;i++){
	buttonsArray.push(allButtons[i])
}
for(let i = 0;i < allTexts.length;i++){
	textsArray.push(allTexts[i])
}
for(let i = 0;i < buttonsArray.length;i++){
	buttonsArray[i].addEventListener('click',function(){
	if(textsArray[i].className === 'services__text'){
		textsArray[i].className = 'services__text:active'
		buttonsArray[i].textContent = 'Нажмите чтобы свернуть'
	} else{
		textsArray[i].className = 'services__text'
		buttonsArray[i].textContent = 'Нажмите чтоб показать'
	}
	})
}

let textStyle = document.getElementsByClassName('services__text')
let textMass = []
for(let i = 0; i < textStyle.length; i++){
	textMass.push(textStyle[i])
}

for( let i = 0; i < textMass.length;i++){
	textMass[i].setAttribute('style', 'background: rgb(225, 204, 79, 0.7); text-align: center; border-radius: 14px; box-shadow: 0 0 10px rgb(0, 0, 0, 0.6);')
}


let btnScrollDown = document.getElementById('contacts')

function scrollDown() {
    let windowCoords = document.documentElement.clientHeight*3;
    (function scroll() {
